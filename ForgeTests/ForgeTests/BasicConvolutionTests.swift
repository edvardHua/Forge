...
class BasicConvolutionTests {
    func runTest(kernel: (Int, Int),
                 ...
                 stride: Int,
                 filter: MPSCNNNeuron?,
                 runMode: Int = 0) { // 这里，0 是表示两个 conv 实现的都运行，而 1 表示只运行自己实现的，2 表示只运行系统实现的
    ...
    func testSpeed(){
        ...
        runTest(kernel: (3, 3), imageSize: (480, 360), inputChannels: 4, outputChannels: 8, stride: 2, filter: filter, runMode: 2)
    }
